...
class SyncManager {
  ...
  setupSocketEventListeners() {
    this.socket.on("connect", () => {
      ...
      if (this.currentUser && !this.currentUser.offline) {
        ...
        console.log(
          `👤 Joined real-time updates for: ${this.currentUser.username}`,
        );
      }
    });
  ...
  scheduleReconnect() {
    ...
    console.log(
      `🔄 Scheduling reconnect attempt ${this.connectionAttempts} in ${this.reconnectDelay}ms`,
    );
  ...
  async getLeaderboard() {
    try {
      const response = await this.fetchWithTimeout(`/leaderboard`);
  ...
  async initializeUser() {
    try {
      ...
      console.warn(
        "Failed to initialize user online, using offline mode:",
        error,
      );
  ...
  async quickSync(type, data) {
    ...
    try {
      const result = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error("Quick sync timeout")), 3000),
        ),
      ]);
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        const result = await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("Persistent sync timeout")),
              10000,
            ),
          ),
        ]);
  ...
  async syncProgress() {
    ...
    const response = await this.fetchWithTimeout(
      `/users/${this.currentUser.id}/progress/sync`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async syncBookmarks() {
    ...
    const response = await this.fetchWithTimeout(
      `/users/${this.currentUser.id}/bookmarks/sync`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  updateLocalProgressFromSync(serverProgress) {
    ...
    for (const progress of serverProgress) {
      ...
      localStorage.setItem(
        `lesson_progress_${progress.lesson_id}`,
        JSON.stringify({
  ...
  getLocalBookmarksForSync() {
    ...
    const lessonBookmarks = JSON.parse(
      localStorage.getItem("bookmarked_lessons") || "[]",
    );
    ...
    // Get item bookmarks
    const itemBookmarks = JSON.parse(
      localStorage.getItem("item_bookmarks") || "[]",
    );
  ...
  async switchToUsername(newUsername) {
    ...
    try {
      ...
      if (this.socket && this.socket.connected) {
        this.socket.emit("join", { username: this.currentUser.username });
        console.log(
          `👤 Joined real-time updates for new username: ${this.currentUser.username}`,
        );
      }
