...
describe("timeTracking Utilities", () => {
  ...
  describe("calculatePaceMetrics (Phase 2 Logic)", () => {
    ...
    it("should calculate next deadline correctly with zero buffer", () => {
      ...
      const expectedDeadline = new Date(
        MOCK_CURRENT_DATE.getTime() + metrics.hoursPerLesson * 60 * 60 * 1000,
      );
      expect(metrics.nextDeadline.getTime()).toBeCloseTo(
    ...
    it("should apply a positive buffer to make the deadline later", () => {
      ...
      expect(metrics.nextDeadline.getTime()).toBeCloseTo(
        expectedDeadlineWithBuffer.getTime(),
      );
    });
  });
  ...
  describe("getEncouragementMessage", () => {
    it("should return correct message for 90%+ completion", () => {
      expect(getEncouragementMessage(0.95)).toBe(
        "🎉 Almost there! You're in the final stretch!",
      );
    });

    it("should return correct message for 75%+ completion", () => {
      expect(getEncouragementMessage(0.8)).toBe(
        "💪 Great progress! Keep up the momentum!",
      );
    });

    it("should return correct message for 50%+ completion", () => {
      expect(getEncouragementMessage(0.6)).toBe(
        "🚀 Halfway there! You're building solid foundations!",
      );
    });

    it("should return correct message for 25%+ completion", () => {
      expect(getEncouragementMessage(0.3)).toBe(
        "📈 Good start! Every lesson counts!",
      );
    });

    it("should return correct message for low but non-zero completion", () => {
      expect(getEncouragementMessage(0.05)).toBe(
        "🌱 Just getting started! The journey begins with a single step!",
      );
    });

    it("should return correct message for zero completion", () => {
      expect(getEncouragementMessage(0)).toBe(
        "🎯 Ready to begin your AP Statistics journey!",
      );
    });
  });
