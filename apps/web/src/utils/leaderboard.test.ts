...
describe('Leaderboard Utilities - MSW Integration', () => {
  describe('fetchLeaderboardData', () => {
    ...
    it('should include current user in MSW response when specified', async () => {
      ...
      const currentUser = result.leaderboard.find(entry => entry.username === 'mocked-user-123');
    ...
    it('should add current user to offline fallback when not in list', async () => {
      ...
      const newUser = result.leaderboard.find(entry => entry.username === 'NewUser123');
    ...
    it('should not duplicate user in offline fallback if already exists', async () => {
      ...
      const existingUser = result.leaderboard.find(entry => entry.username === 'StudyBot');
  ...
  describe('findUserRank', () => {
    const mockLeaderboard: LeaderboardEntry[] = [
      { rank: 1, username: 'TopPlayer', completed_videos: 10, completed_quizzes: 8, total_completed: 18 },
      { rank: 2, username: 'SecondPlace', completed_videos: 8, completed_quizzes: 6, total_completed: 14 },
      { rank: 3, username: 'ThirdPlace', completed_videos: 5, completed_quizzes: 4, total_completed: 9 },
    ];
