...
describe('Auth Utilities', () => {
  ...
  describe('generateFallbackUsername', () => {
    it('should generate username with correct format', () => {
      ...
      expect(username).toMatch(/^(happy|clever|bright|swift|calm)(panda|fox|owl|cat|wolf)\d+$/);
    });
  ...
  describe('parseStoredUser', () => {
    ...
    it('should return null for invalid user objects', () => {
      ...
      expect(authUtils.parseStoredUser(JSON.stringify({ id: 'not-a-number' }))).toBe(null);
      expect(authUtils.parseStoredUser(JSON.stringify({ username: 'test' }))).toBe(null); // missing id
