...
export function getTotalItemCounts(
  allUnitsDataArray: Unit[] = ALL_UNITS_DATA,
  ...
  if (!allUnitsDataArray || !Array.isArray(allUnitsDataArray)) {
    console.error(
      "Invalid data provided to getTotalItemCounts in allUnitsData.ts",
    );
...
export function calculateTopicFraction(
  ...
  if (videoWeight < 0) {
    throw new Error(
      `Dynamic weight calculation error: video weight would be negative (${videoWeight})`,
    );
  }
  ...
  if (Math.abs(totalWeight - 1.0) > 0.001) {
    throw new Error(
      `Dynamic weights must sum to 1.0, but sum to ${totalWeight}`,
    );
  }
...
export function calculateTotalFractionalLessons(
  ...
  return allTopics.reduce((total, topic) => {
    const userProgressData = userProgressArray.find(
      (p) => p.lesson_id === topic.id,
    );
