...
test.describe("Diagnostic Tests", () => {
  test("should load the application with MSW enabled and show mocked content", async ({
    page,
  }) => {
    ...
    await page.screenshot({
      path: "test-results/diagnostic-msw-screenshot.png",
    });
    ...
    const hasMockedUsername =
      (await page.getByText("mocked-user-123").count()) > 0;
  ...
  test("should compare MSW vs non-MSW behavior", async ({ page }) => {
    ...
    console.log(
      "Non-MSW has mocked-user-123:",
      nonMswContent?.includes("mocked-user-123"),
    );

    if (nonMswButtons > 0) {
      const firstButtonText = await page
        .locator("button")
        .first()
        .textContent();
    ...
    console.log(
      "MSW has mocked-user-123:",
      mswContent?.includes("mocked-user-123"),
    );
