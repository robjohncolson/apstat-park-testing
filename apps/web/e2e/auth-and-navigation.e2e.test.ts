...
test.describe("Full User Journey (Happy Path)", () => {
  test("should allow a user to log in, view the dashboard, and see the leaderboard", async ({
    page,
  }) => {
    ...
    await page
      .getByRole("button", { name: /Continue as mocked-user-123/ })
      .click();
    ...
    await expect(
      page.getByRole("heading", { name: /Welcome, mocked-user-123/ }),
    ).toBeVisible();
    await expect(
  ...
  test("should handle username generation and custom username input", async ({
    page,
  }) => {
  ...
  test("should navigate back from leaderboard to dashboard", async ({
    page,
  }) => {
