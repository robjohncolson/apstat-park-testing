...
test.describe('Basic User Flow (Real API)', () => {
  test('should load the login page and show generated username', async ({ page }) => {
    // Navigate to the app
    await page.goto('/');
    ...
    await expect(page.getByRole('heading', { name: /Welcome to APStat Park/ })).toBeVisible();
    
    // Check that a username was generated (should be some random username)
    const continueButtons = await page.locator('button:has-text("Continue as")').count();
    expect(continueButtons).toBeGreaterThan(0);
    
    // Get the generated username from the button text
    const firstContinueButton = page.locator('button:has-text("Continue as")').first();
  ...
  test('should allow login with generated username and navigate to dashboard', async ({ page }) => {
    // Navigate to the app
    await page.goto('/');
    ...
    const continueButton = page.locator('button:has-text("Continue as")').first();
  ...
  test('should navigate from dashboard to leaderboard and back', async ({ page }) => {
    // Complete login flow
    await page.goto('/');
    await page.waitForTimeout(3000);
