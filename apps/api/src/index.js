...
function broadcastToUser(username, message) {
  ...
  if (userSockets && userSockets.size > 0) {
    ...
    console.log(
      `📤 Broadcasted to ${userSockets.size} devices for user: ${username}`,
    );
  }
}
...
io.on("connection", (socket) => {
  ...
  socket.on("join", ({ username }) => {
    ...
    console.log(
      `👤 User ${username} connected (${connectedUsers.get(username).size} devices)`,
    );
  ...
  socket.on("user_activity", async (activity) => {
    ...
    try {
      ...
      await pool.query(
        `NOTIFY user_activity, '${activityPayload.replace(/'/g, "''")}'`,
      );
    } catch (error) {
  ...
  try {
    ...
    if (username) {
      ...
      try {
        ...
        console.log(
          "User created successfully with custom username:",
          insertResult.rows[0],
        );
...
app.post("/api/users/:userId/progress/sync", async (req, res) => {
  try {
    ...
    try {
        ...
        if (existingResult.rows.length > 0) {
          ...
          const mergedVideos = [
            ...new Set([
              ...
              ...localProgress.videosWatched,
            ]),
          ];
          const mergedQuizzes = [
            ...new Set([
              ...existing.quizzes_completed,
              ...localProgress.quizzesCompleted,
            ]),
          ];
          const lessonCompleted =
            existing.lesson_completed || localProgress.lessonCompleted;
          const completedAt = lessonCompleted
            ? localProgress.completedAt &&
              new Date(localProgress.completedAt) >
      ...
      await client.query("UPDATE users SET last_sync = NOW() WHERE id = $1", [
        userId,
      ]);
      ...
      await pool.query(
        `NOTIFY progress_updates, '${notificationPayload.replace(/'/g, "''")}'`,
      );
...
app.post("/api/users/:userId/bookmarks/sync", async (req, res) => {
  try {
    ...
    try {
      ...
      await pool.query(
        `NOTIFY bookmark_updates, '${bookmarkNotificationPayload.replace(/'/g, "''")}'`,
      );
...
app.post("/api/users/:userId/gold-star", async (req, res) => {
  try {
    ...
    if (!lessonId || !completionTime) {
      return res
        .status(400)
        .json({ error: "Missing lessonId or completionTime" });
    }
    ...
    const hoursPerLesson =
      remainingLessons > 0 ? (daysUntilExam * 24) / remainingLessons : 0;
    ...
    const nextTargetHours =
      nextRemainingLessons > 0
        ? (daysUntilExam * 24) / nextRemainingLessons
        : 0;
    ...
    await pool.query(
      `NOTIFY leaderboard_updates, '${leaderboardPayload.replace(/'/g, "''")}'`,
    );
...
app.post("/api/users/:userId/reset", async (req, res) => {
  try {
    ...
    try {
      ...
      const goldStarResult = await client.query(
        "DELETE FROM gold_stars WHERE user_id = $1",
        [userId],
      );
...
async function initializeDatabase() {
  try {
    ...
    await pool.query(
      `CREATE INDEX IF NOT EXISTS idx_gold_stars_user ON gold_stars(user_id);`,
    );
    await pool.query(
      `CREATE INDEX IF NOT EXISTS idx_gold_stars_leaderboard ON gold_stars(current_streak DESC, total_stars DESC);`,
    );
...
server.listen(port, () => {
  ...
  console.log(
    `Database URL: ${process.env.DATABASE_URL ? "Set from environment" : "Using fallback"}`,
  );
});
